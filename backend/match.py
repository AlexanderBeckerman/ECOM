...
def rank_restaurants(customer_scores, restaurants, categories, customer_location=None, max_threshold=3,
                     min_threshold=0.3):
    rankings = []
    ...
    for restaurant in restaurants:
        ...
        restaurant_scores = [temp.get(category, 3) for category in
                             categories]  # set default score to 3 if category not found
        ...
        if np.any(differences > max_threshold):  # If any category difference is over the threshold: set score to -inf
            score = -999999
            rankings.append((restaurant.name, score, []))
            continue

        similar_categories = [categories[i] for i in range(len(differences)) if
                              differences[i] <= min_threshold]  # mark categories that are similar
        ...
        rankings.append((restaurant.name, score, similar_categories))
...
def match_restaurants_to_user(user_scores, categories, location=None):
    ...
    if not restaurant_cache:
        with Session() as session:
            restaurants = session.query(Restaurant).all()
            restaurant_cache = restaurants

    results = rank_restaurants(user_scores, restaurant_cache, categories=categories, customer_location=location)
    return results
